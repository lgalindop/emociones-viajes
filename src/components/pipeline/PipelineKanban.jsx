...
import { Plus, Filter, User, Calendar, DollarSign } from "lucide-react";
import ConvertToSale from "../sales/ConvertToSale";

const STAGES = [
  { key: 'lead', label: 'Lead', color: 'bg-gray-100 border-gray-300', prob: 25 },
  { key: 'qualification', label: 'Calificación', color: 'bg-blue-100 border-blue-300', prob: 35 },
  { key: 'quote_sent', label: 'Cotización Enviada', color: 'bg-purple-100 border-purple-300', prob: 50 },
  { key: 'negotiation', label: 'Negociación', color: 'bg-yellow-100 border-yellow-300', prob: 65 },
  { key: 'booking_confirmed', label: 'Reserva Confirmada', color: 'bg-green-100 border-green-300', prob: 90 },
  { key: 'payment_pending', label: 'Pago Pendiente', color: 'bg-orange-100 border-orange-300', prob: 95 },
  { key: 'fully_paid', label: 'Pagado', color: 'bg-teal-100 border-teal-300', prob: 100 },
];
...
export default function PipelineKanban({ onNewQuote }) {
  ...
  async function handleDrop(e, targetStage) {
    ...
    if (targetStage === 'booking_confirmed' && draggedCard.pipeline_stage !== 'booking_confirmed') {
      ...
      <div className="max-w-[1800px] mx-auto">
        ...
        <div className="flex gap-4 overflow-x-auto pb-4">
          {STAGES.map(stage => {
            ...
            const stageValue = stageCards.reduce((sum, c) => sum + (c.presupuesto_aprox || 0), 0);
