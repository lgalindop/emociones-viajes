...
export default function DetallesCotizacion({ cotizacionId, onBack, onDeleted }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      const result1 = await supabase.from('cotizaciones').select('*').eq('id', cotizacionId).single()
      if (result1.error) throw result1.error
      setCotizacion(result1.data)
      setEditData(result1.data)

      const result2 = await supabase.from('opciones_cotizacion').select('*').eq('cotizacion_id', cotizacionId)
      if (result2.error) throw result2.error
      setOpciones(result2.data || [])
      setEditingOpciones(result2.data || [])

      const result3 = await supabase.from('operadores').select('*').eq('activo', true)
  ...
  async function handleSave() {
    try {
      const result = await supabase.from('cotizaciones').update(editData).eq('id', cotizacionId)
      ...
      if (opcionesAEliminar.length > 0) {
        const deleteResult = await supabase.from('opciones_cotizacion').delete().in('id', opcionesAEliminar)
      ...
      for (const op of editingOpciones) {
        ...
        if (op.id) {
          const updateResult = await supabase.from('opciones_cotizacion').update(opcionData).eq('id', op.id)
          if (updateResult.error) throw updateResult.error
        } else {
          const insertResult = await supabase.from('opciones_cotizacion').insert([opcionData])
  ...
  async function handleDelete() {
    ...
    try {
      const result = await supabase.from('cotizaciones').delete().eq('id', cotizacionId)
      if (result.error) throw result.error
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-6xl mx-auto">
        <button onClick={onBack} className="flex items-center gap-2 text-gray-600 hover:text-primary mb-6">
          <ArrowLeft size={20} />
          Regresar
        </button>
            ...
            <div className="flex gap-2">
              {!editing && (
                <>
                  ...
                  />
                  <button onClick={() => alert('PDF export - próximamente')} className="flex items-center gap-2 px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700">
                    <Download size={18} />
                    PDF
                  </button>
          ...
          <div className="space-y-6">
            <div className="border-l-4 border-primary pl-4">
              ...
              {!editing ? (
                ...
                <div className="space-y-4">
                  <input type="text" value={editData.cliente_nombre} onChange={(e) => setEditData({...editData, cliente_nombre: e.target.value})} className="w-full border rounded-lg px-4 py-2" placeholder="Nombre" />
                  <div className="grid grid-cols-2 gap-4">
                    <input type="tel" value={editData.cliente_telefono || ''} onChange={(e) => setEditData({...editData, cliente_telefono: e.target.value})} className="w-full border rounded-lg px-4 py-2" placeholder="Teléfono" />
                    <input type="email" value={editData.cliente_email || ''} onChange={(e) => setEditData({...editData, cliente_email: e.target.value})} className="w-full border rounded-lg px-4 py-2" placeholder="Email" />
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="border-l-4 border-primary-light pl-4">
              ...
              {!editing ? (
                ...
                <div className="space-y-4">
                  <input type="text" value={editData.destino} onChange={(e) => setEditData({...editData, destino: e.target.value})} className="w-full border rounded-lg px-4 py-2" placeholder="Destino" />
                  <div className="grid grid-cols-2 gap-4">
                    <input type="date" value={editData.fecha_salida} onChange={(e) => setEditData({...editData, fecha_salida: e.target.value})} className="w-full border rounded-lg px-4 py-2" />
                    <input type="date" value={editData.fecha_regreso} onChange={(e) => setEditData({...editData, fecha_regreso: e.target.value})} className="w-full border rounded-lg px-4 py-2" />
                  </div>
                  <div className="grid grid-cols-3 gap-4">
                    <input type="number" value={editData.num_adultos} onChange={(e) => setEditData({...editData, num_adultos: parseInt(e.target.value)})} className="w-full border rounded-lg px-4 py-2" placeholder="Adultos" />
                    <input type="number" value={editData.num_ninos} onChange={(e) => setEditData({...editData, num_ninos: parseInt(e.target.value)})} className="w-full border rounded-lg px-4 py-2" placeholder="Niños" />
                    <input type="number" value={editData.presupuesto_aprox || ''} onChange={(e) => setEditData({...editData, presupuesto_aprox: e.target.value})} className="w-full border rounded-lg px-4 py-2" placeholder="Presupuesto" />
                  </div>
                  <textarea value={editData.requerimientos || ''} onChange={(e) => setEditData({...editData, requerimientos: e.target.value})} className="w-full border rounded-lg px-4 py-2" rows="2" placeholder="Requerimientos" />
                  <textarea value={editData.notas || ''} onChange={(e) => setEditData({...editData, notas: e.target.value})} className="w-full border rounded-lg px-4 py-2" rows="2" placeholder="Notas internas" />
                </div>
              )}
            </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-4">
                <h3 className="font-semibold text-gray-700">Opciones de Paquetes ({editing ? editingOpciones.length : opciones.length})</h3>
              </div>
                  ...
                  return (
                    <div key={op.id || op.temp_id || idx} className="border rounded-lg p-6 bg-gray-50">
                        ...
                        <div className="bg-blue-50 border-l-4 border-blue-500 p-3 mb-4 text-sm">
                          <div className="grid grid-cols-3 gap-2">
                            ...
                            <div>
                              ...
                              {diferencia >= 0 ? (
                                <p className="text-xs text-green-600">Dentro de presupuesto</p>
                              ) : (
                      ...
                      <div className="flex justify-between items-start mb-4">
                        ...
                        <span className="text-2xl font-bold text-primary">${precioOpcion.toLocaleString('es-MX')}</span>
                      </div>
                      ...
                      {op.precio_por_persona > 0 && (
                        <p className="text-sm text-gray-600 mt-2">Precio por persona: ${parseFloat(op.precio_por_persona).toLocaleString('es-MX')}</p>
                      )}
                      ...
                      {incluyeArray.length > 0 && (
                        <div className="mt-4">
                          ...
                          <ul className="text-sm text-gray-600 ml-4 mt-1 list-disc">
                            {incluyeArray.map((item, i) => <li key={i}>{item.trim()}</li>)}
                          </ul>
                        </div>
                      )}
                      ...
                      {noIncluyeArray.length > 0 && (
                        <div className="mt-3">
                          ...
                          <ul className="text-sm text-gray-600 ml-4 mt-1 list-disc">
                            {noIncluyeArray.map((item, i) => <li key={i}>{item.trim()}</li>)}
                          </ul>
                        </div>
                      )}

                      {op.disponibilidad && <p className="text-sm text-gray-500 mt-3 italic">{op.disponibilidad}</p>}

                      {op.link_paquete && (
                        <a href={op.link_paquete} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-600 hover:underline mt-2 inline-block">
